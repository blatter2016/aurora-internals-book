...
InnoDB assigns every read-write transaction a 48-bit unsigned integer identifier (`trx_id_t`). The maximum value is 2^48 − 1 = 281,474,976,710,655. At a sustained rate of 20,000 transactions per second, wraparound would require approximately 446 years; in practice, transaction ID exhaustion is not an operational concern.
...
The write margin (`TRX_SYS_TRX_ID_WRITE_MARGIN = 256`) ensures that `max_trx_id` is flushed to the rollback segment header in the redo log every 256 assignments. After a crash, the recovered `max_trx_id` is at least as large as any ID that was assigned to a committed transaction, preventing ID reuse that would corrupt MVCC visibility.
...
The `trx_sys_t` structure is the single global object that coordinates all transaction activity in InnoDB. Its key fields are:
...
- **REPEATABLE READ** (the default): `trx_assign_read_view()` is called once, at the first `SELECT` within the transaction. The same read view is reused for every subsequent consistent read in that transaction. If a transaction starts with an `UPDATE` or `DELETE` and then issues a `SELECT`, the read view is created at that `SELECT` and held until `COMMIT` or `ROLLBACK`.

- **READ COMMITTED**: `trx_assign_read_view()` is called at the start of every `SELECT` statement, creating a fresh read view each time. Read views are short-lived and closed promptly after the statement completes.
...
4. Apply the update vector to the current record copy using `row_upd_rec_in_place()`, producing the historical version.
...
- `TRX_UNDO_DEL_MARK_REC` — a delete-mark operation. The row has been marked as deleted but not yet physically removed. When purge processes this record, it physically removes the row from both the clustered index and all secondary indexes.
...
**Consistent reads** (non-locking reads) are the default for `SELECT` at `READ COMMITTED` and `REPEATABLE READ`. They use MVCC and read views to access a snapshot without acquiring locks on the rows read. Other sessions can freely modify the same rows while the consistent read is in progress.
...
Locking reads are used internally by `UPDATE`, `DELETE`, and `INSERT... ON DUPLICATE KEY UPDATE`. If a locking read encounters a row modified by an uncommitted transaction, it waits until that transaction commits or rolls back. At `SERIALIZABLE`, all plain `SELECT` statements are implicitly converted to `SELECT... FOR SHARE` (when autocommit is disabled), abandoning MVCC entirely.
