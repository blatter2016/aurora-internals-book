...
| `AuroraReplicaLag` | 1 (Cause) | Leading | Reader buffer pool pressure; cache-vs-transaction distinction | > 1,000 ms warning; > 10,000 ms critical |
| `CPUUtilization` | 2 (Symptom) | Lagging | Consumption, not cause; requires wait event analysis [^497^] | > 80% for 5 min |
| `DatabaseConnections` | 2 (Symptom) | Lagging | Result of slow queries, not root cause | > 80% of practical limit |
...
Aurora MySQL publishes metrics to CloudWatch in the `AWS/RDS` namespace at 1-minute granularity by default, retained for 15 days (extendable via Enhanced Monitoring and Database Insights Advanced mode). The selection of metrics to alert on — and their thresholds — separates reactive firefighting from proactive operations.
...
| 3 | `aurora-replica-lag-high` | `AuroraReplicaLagMaximum` | > 1,000 ms | P2 (High) | 3 min | Worst replica falling behind; memory pressure or network issue |
| 4 | `aurora-cpu-high` | `CPUUtilization` | > 80% | P3 (Medium) | 5 min | CPU saturation; investigate queries via Database Insights [^497^] |
| 5 | `aurora-memory-low` | `FreeableMemory` | < 20% of total RAM | P2 (High) | 5 min | Memory pressure causes cache evictions, then I/O, then latency |
| 6 | `aurora-connections-high` | `DatabaseConnections` | > 80% of practical limit | P2 (High) | 3 min | Connection pool exhaustion; check for leaked connections or slow queries |
...
| 8 | `aurora-login-failures` | `LoginFailures` | > 0 | P3 (Medium) | 1 min | Security or credential issue; immediate investigation |
| 9 | `aurora-volume-read-iops-spike` | `VolumeReadIOPS` | > 3x baseline | P3 (Medium) | 5 min | Cache miss spike or full table scan; correlate with BufferCacheHitRatio |
...
The `--ok-actions` parameter ensures recovery notifications when metrics return to normal. Without it, on-call engineers must manually verify recovery, adding noise to incident response.

### VolumeReadIOPS: The Cache Miss Signal

`VolumeReadIOPS` measures billed read I/O from the cluster storage volume. In a healthy cluster this stays under 100 during normal operations. Spikes indicate either buffer pool cache misses (working set exceeds memory, Chapter 3) or full table scans (queries reading entire tables without indexes).

When `VolumeReadIOPS` spikes, check `BufferCacheHitRatio`. If the hit ratio dropped simultaneously, the working set exceeds buffer pool capacity — scale up to a larger instance. If the hit ratio remained stable, a specific query is performing a full table scan; find it via Database Insights Top SQL or the slow query log.
...
Enhanced Monitoring collects OS metrics directly from an agent on the DB instance, bypassing the hypervisor layer that standard CloudWatch uses. This produces more accurate CPU and memory measurements, especially on smaller instances.
...
- **No reboot required**: enabling or changing granularity does not restart the instance.
- **Minimal overhead**: the agent runs on the host OS, not inside the database engine.

Enhanced Monitoring captures CPU breakdown (user, system, iowait, steal), memory (free, cached, buffers, active, inactive), disk I/O per device, network packets, swap usage, and per-process resource consumption. The process list appears in Database Insights Advanced mode under the OS Process tab, distinguishing mysqld usage from background Aurora processes (csdd, HM).
...
The IAM role requires the `AmazonRDSEnhancedMonitoringRole` policy. Each instance creates a log stream named after its `resourceId` within `RDSOSMetrics`.

### Log Types and Their Use Cases

Aurora MySQL supports four log types. All can be published to CloudWatch Logs for centralized querying.
...
| General log | Disabled | `/aws/rds/instance/<id>/general` | Complete SQL audit for debugging | Short windows only; high I/O overhead |
| Audit log | Disabled | `/aws/rds/instance/<id>/audit` | Compliance and security via MariaDB Audit Plugin | For regulatory requirements |
...
Supported events: `CONNECT`, `QUERY` (all queries in plain text), `QUERY_DCL`, `QUERY_DDL`, `QUERY_DML`, and `TABLE`.

### CloudWatch Logs Insights: Sample Queries

Set log retention to 30 days for production; export to S3 for long-term archival.
...
Datadog provides the deepest Aurora MySQL integration among commercial platforms. Its Database Monitoring captures query-level metrics, execution plans, wait event analysis, InnoDB telemetry, and connection data. It autodiscovers Aurora endpoints and correlates database metrics with AWS infrastructure data.

Critical configuration: the Datadog Agent must connect to each **instance endpoint**, not the cluster endpoint. Connecting through the cluster endpoint collects data from a random replica each time, producing inconsistent metrics.
...
The Prometheus `mysqld_exporter` collects MySQL metrics for scraping at port 9104, requiring a monitoring user with `SELECT` privileges.
...
Grafana Cloud offers Database Observability via Grafana Alloy, which connects to Aurora instance endpoints and streams metrics to Grafana Cloud. Requirements: Aurora MySQL 8.0+, `performance_schema = ON`, `performance_schema_max_digest_length = 4096`. The open-source stack requires more setup but provides full data ownership and avoids per-vCPU pricing.

### Percona Monitoring and Management (PMM)

PMM is a free, open-source tool supporting Aurora MySQL via AWS discovery. It provides query analytics (QAN), alerting, and dashboards. OS-level metrics require Enhanced Monitoring and IAM permissions (`cloudwatch:GetMetricData`, `rds:DescribeDBInstances`). A common issue is missing IAM permissions causing blank OS metric graphs. PMM's Query Analytics parses slow query logs and Performance Schema to identify top queries by time, frequency, and resource consumption.
