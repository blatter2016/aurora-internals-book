...
Aurora PostgreSQL has Cluster Cache Management (CCM), which transfers cache state to the failover target — the promoted reader has a pre-warmed buffer pool [^334^]. Aurora MySQL has no equivalent. Survivable page cache keeps the *same* instance warm across restarts, but when a *different* instance is promoted, it starts cold. Expect 30-50% degradation for 10-30 minutes after Aurora MySQL failovers [^334^].
...
Aurora PostgreSQL provides Query Plan Management (QPM) through the `apg_plan_mgmt` extension, which captures, approves, and locks query plans — preventing regressions across upgrades. Aurora MySQL has no equivalent. Plan stability depends on hints, `STRAIGHT_JOIN`, and `ANALYZE TABLE` — all requiring manual maintenance.

The 2.x to 3.x upgrade makes this acute. Aurora MySQL 2.x had query cache, which fixed execution paths for repeated queries. Query cache was removed in 3.x, creating a double regression: no query cache for speed, and no QPM for stability. Teams upgrading must establish application-level caching (Redis, Memcached) and a query hint library before the upgrade.
...
| Query plan stability | Hints, STRAIGHT_JOIN, manual ANALYZE | QPM extension: capture, approve, lock plans | Choose PostgreSQL for workloads with complex plans sensitive to statistics |
...
| Upgrade path | 2.x→3.x loses query cache; no QPM replacement | QPM persists across upgrades | Plan application caching and hint library before MySQL 3.x upgrade |
...
| Monitor replication lag | Check `Seconds_Behind_Master` | Use `AuroraReplicaLag` (cache lag, not transaction lag) |
...
| High I/O remediation | Tune `innodb_io_capacity` | Locked at 200/2000; fix purge or buffer pool instead |
...
| Parameter tuning | Adjust 20+ storage and I/O parameters | Focus on buffer pool, purge threads, locks, connections; rest is automated |
